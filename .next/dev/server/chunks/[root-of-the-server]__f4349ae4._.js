...
async function GET(request) {
    try {
        ...
        const [clientesSnapshot, leadsSnapshot, vendasSnapshot] = await Promise.all([
            __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$firebase$2f$admin$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["collections"].clientes().where('userId', '==', user.id).get(),
            __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$firebase$2f$admin$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["collections"].leads().where('userId', '==', user.id).get(),
            __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$firebase$2f$admin$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["collections"].vendas().where('userId', '==', user.id).get()
        ]);
        // Contar totais
        const totalClientes = clientesSnapshot.size;
        const totalLeads = leadsSnapshot.docs.filter((doc)=>doc.data().status !== 'convertido' && doc.data().status !== 'perdido').length;
        // Calcular vendas do mês atual
        const agora = new Date();
        const inicioMes = new Date(agora.getFullYear(), agora.getMonth(), 1);
        const vendasMes = vendasSnapshot.docs.filter((doc)=>{
        ...
        const leadsConvertidos = leadsSnapshot.docs.filter((doc)=>doc.data().status === 'convertido').length;
        const totalLeadsHistorico = leadsSnapshot.size;
        const taxaConversao = totalLeadsHistorico > 0 ? Math.round(leadsConvertidos / totalLeadsHistorico * 100) : 0;
        // Calcular crescimento (comparar com mês anterior)
        const inicioMesAnterior = new Date(agora.getFullYear(), agora.getMonth() - 1, 1);
        const fimMesAnterior = new Date(agora.getFullYear(), agora.getMonth(), 0);
        const vendasMesAnterior = vendasSnapshot.docs.filter((doc)=>{
            const venda = doc.data();
            const dataVenda = new Date(venda.dataVenda || venda.criadoEm);
            return dataVenda >= inicioMesAnterior && dataVenda <= fimMesAnterior && venda.status === 'pago';
        }).reduce((total, doc)=>total + (doc.data().valor || 0), 0);
        const crescimentoVendas = vendasMesAnterior > 0 ? Math.round((vendasMes - vendasMesAnterior) / vendasMesAnterior * 100) : 0;
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: true,
            totalClientes,
            totalLeads,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f4349ae4._.js.map